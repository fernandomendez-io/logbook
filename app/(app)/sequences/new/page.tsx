...
export default function NewSequencePage() {
  ...
  async function handleSave() {
    ...
    const res = await fetch("/api/sequences", {
      ...
      body: JSON.stringify({
        rawText,
        yearMonth,
        confirmedFlights: parsed.allFlights,
      }),
    });
    const data = await res.json();
    setSaving(false);
  ...
  const activeFlights =
    parsed?.allFlights.filter((f) => !f.isCancelled && !f.isDeadhead) ?? [];
      ...
      <Card>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-foreground/30 mt-1">
            HSS shows day numbers only — this tells the parser which month they
            fall in.
          </p>
        </div>
        ...
        <Textarea
          ...
          rows={14}
          placeholder={`SEQ 0067 BASE DFW SEL 000 DOM E75\nCAPT COOL CAPTAIN EMP NBR 000000\nF/O CAPTAIN IN TRAINING EMP NBR 0000001\n DT EQ FLT STA DEP STA ARR AC FLY GTR GRD ACT\nSKD 15 54 0001 DFW 0710 ACT 0825 1.15 0.30\nACT 15 1A 3779 DFW 0704 ACT 0812 1.08 1.15 0.38\n...`}
        ...
        <div className="flex gap-3 mt-4">
          <Button
            onClick={handleParse}
            loading={parsing}
            disabled={!rawText.trim() || !yearMonth}
          >
            Parse Sequence
          </Button>
          {parsed && (
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-mono font-bold text-green-primary">
              ...
              <div>
                ...
                <p className="font-mono">
                  {parsed.tafbHrs ? fmtHrs(parsed.tafbHrs) : "—"}
                </p>
              </div>
              {parsed.captainName && (
                <div>
              ...
              <div>
                ...
                <p className="font-mono">
                  {parsed.sequenceCreditHrs
                    ? fmtHrs(parsed.sequenceCreditHrs)
                    : "—"}
                </p>
              </div>
            </div>
            <div className="flex gap-3 mt-4 pt-4 border-t border-border flex-wrap">
              ...
              {deadheads.length > 0 && (
                <Badge variant="blue">{deadheads.length} deadheads</Badge>
              )}
              {cancelled.length > 0 && (
                <Badge variant="red">{cancelled.length} cancelled/RTD</Badge>
              )}
              {parsed.events.length > 0 && (
                <Badge variant="yellow">{parsed.events.length} events</Badge>
              )}
            </div>
          </Card>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {dp.flights.map((f, fIdx) => (
                      <tr
                        key={fIdx}
                        className={f.isCancelled ? "opacity-40" : ""}
                      >
                        ...
                        <td className="py-2 pr-4 text-right font-mono text-xs">
                          {f.actualBlockHrs != null ? (
                            <span
                              className={
                                f.actualBlockHrs > (f.scheduledBlockHrs ?? 0)
                                  ? "text-yellow-400"
                                  : "text-foreground/70"
                              }
                            >
                        ...
                        <td className="py-2">
                          <div className="flex gap-1 flex-wrap">
                            {f.isDeadhead ? (
                              ...
                              <Badge variant="red">
                                {f.isReturnedToGate ? "RTD" : "CXL"}
                              </Badge>
                            ) : (
                              <Badge variant="green">Flight</Badge>
                            )}
                            {f.isDiverted && (
                              <Badge variant="yellow">DIV</Badge>
                            )}
