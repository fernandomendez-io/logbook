...
export default async function SequencesPage() {
  ...
  for (const f of (flightCounts ?? [])) {
    if (!countsMap[f.sequence_id]) countsMap[f.sequence_id] = { total: 0, active: 0 }
    countsMap[f.sequence_id].total++
    if (!f.is_cancelled) countsMap[f.sequence_id].active++
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/50 mt-1">{sequences?.length || 0} total</p>
        </div>
      ...
      {!sequences?.length ? (
        <Card className="text-center py-16">
          <div className="w-12 h-12 bg-green-primary/10 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-6 h-6 text-green-primary" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          {upcoming.length > 0 && (
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-foreground/40">{upcoming.length} sequence{upcoming.length !== 1 ? 's' : ''}</span>
              </div>
                  ...
                  return (
                    <Link key={seq.id} href={`/sequences/${seq.id}`}>
                      <div className="bg-surface border border-border rounded-lg px-5 py-4 hover:border-green-dim transition-colors flex items-center gap-4">
                        <div className="font-mono font-bold text-green-primary text-lg w-20">{seq.sequence_number}</div>
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-medium">{seq.domicile}</span>
                            <span className="text-foreground/30">·</span>
                            <span className="text-sm text-foreground/60">{formatDate(seq.report_date)} → {formatDate(seq.release_date)}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge variant="green">{c.active} legs</Badge>
                          <Badge variant={seq.status === 'active' ? 'outline' : 'gray'}>{seq.status}</Badge>
                        </div>
                        <svg className="w-4 h-4 text-foreground/30" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          {past.length > 0 && (
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-foreground/40">{past.length} sequence{past.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="space-y-2">
                {past.map(seq => {
                  const c = countsMap[seq.id] ?? { active: 0 }
