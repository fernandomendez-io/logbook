...
interface FR24SummaryResponse { data: FR24SummaryFlight[] }
...
interface FR24EventFlight { fr24_id: string; events: FR24Event[] }
interface FR24EventResponse { data: FR24EventFlight[] }

async function fr24Fetch(path: string, params: Record<string, string>): Promise<unknown> {
...
export async function fetchACARSTimes(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[FR24] ── Step 1: flight-summary/light ──────────────────────')
      console.dir(summaryRaw, { depth: null })
    }

    const summary = summaryRaw as FR24SummaryResponse | null
    if (!summary?.data?.length) {
      return { times: null, scheduledHint: null, raw: summaryRaw, fr24FlightId: null, selectedIndex: null, totalLegs: 0, status: null }
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[FR24] ${data.length} leg(s) — selected index=${selectedIndex}, fr24_id="${leg.fr24_id}", origin filter="${originIata ?? 'any'}"`)
    }

    const tail   = leg.reg  ?? null
    const acType = mapAircraftType(leg.type)
    const origin = leg.orig_iata
      ?? (leg.orig_icao?.length === 4 ? leg.orig_icao.slice(1) : leg.orig_icao)
      ?? null
    const dest = leg.dest_iata_actual ?? leg.dest_iata
      ?? (leg.dest_icao_actual?.length === 4 ? leg.dest_icao_actual.slice(1) : leg.dest_icao_actual)
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[FR24] ── Step 2: historic/flight-events/light ───────────────')
    ...
    const lastSeenUtc  = sortedByTime[sortedByTime.length - 1]?.timestamp ?? null
    ...
    return {
      times: (hasAirTimes || hasGateTimes) ? {
    ...
    return { times: null, scheduledHint: null, raw: null, fr24FlightId: null, selectedIndex: null, totalLegs: 0, status: null }
  }
}
